...
import type { CurrentWeather as CurrentWeatherType, TempUnit } from "@/types/weather";
import { formatTemp, formatWindSpeed, formatVisibility, getWindDirection, formatTime, capitalize } from "@/lib/weather-utils";
  ...
  return (
    <div className="flex items-center gap-3 bg-muted/40 rounded-xl p-3.5">
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm font-semibold text-foreground truncate" data-testid={`stat-${label.toLowerCase().replace(/\s/g, "-")}`}>{value}</p>
      </div>
    </div>
  );
}
...
export function CurrentWeatherCard({ weather, unit }: CurrentWeatherCardProps) {
  ...
  return (
    <div className="bg-card border border-card-border rounded-2xl shadow-md overflow-hidden" data-testid="current-weather-card">
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent px-6 pt-6 pb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-2xl font-bold text-foreground" data-testid="text-city-name">
                {weather.name}
              </h2>
            ...
            <p className="text-sm text-muted-foreground capitalize" data-testid="text-weather-description">
        ...
        <div className="mt-2 flex items-end gap-4">
          <span className="text-6xl font-bold tracking-tight text-foreground" data-testid="text-temperature">
